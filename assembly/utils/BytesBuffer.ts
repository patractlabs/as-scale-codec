...
import { BYTE_LENGTH, Bytes } from "./Bytes";
...
export class BytesBuffer {
    ...
    */
    encodeCompactInt (i: i64): void {
        if (i < 1 << 6) {
            Bytes.appendUint<u8>(this.bytes, u8(i) << 2, BYTE_LENGTH.INT_8);
        } else if (i < 1 << 14) {
            Bytes.appendUint<u16>(this.bytes, u16(i << 2) + 1, BYTE_LENGTH.INT_16);
        } else if (i < 1 << 30) {
            Bytes.appendUint<u32>(this.bytes, u32(i << 2) + 2, BYTE_LENGTH.INT_32);
        } else {
            ...
            Bytes.putUint<u8>(this.bytes, lengthByte, BYTE_LENGTH.INT_8);
            Bytes.putUint<u64>(o, i64(i), BYTE_LENGTH.INT_64);
